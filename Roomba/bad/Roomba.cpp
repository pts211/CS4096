...
bool Roomba::turn(int degrees)
{
  ...
  {
    if( getSensors(Sensor::ALL) )
    {
      //if( !(sensors.getAngle() != prevAngle) ){
      cout<<"sensor.angle: "<<sensors.getAngle()<<endl;
        total_turned += sensors.getAngle();
      //}
    }
    cout<<"Total turned: "<<total_turned<<endl;
    prevAngle = sensors.getAngle();
  }
  drive(0, 0);
...
bool Roomba::read(int numBytes)
{
  ...
  */
  ...
  return sensors.parseSensorData(pkt_code, rec);
}
